...
#EFFICIENCY METHODS
# predicted FWHM at 1460.8 keV
a = -391.425599
b = 6.723136106
c = -0.000543789

predicted_fwhm = np.sqrt(
    np.maximum(a + b*1460.8 + c*(1460.8**2), 0.1)
)

# Method 1: calibrated + smeared

roi = (
    (centers > cal_k40_centroid - predicted_fwhm) &
    (centers < cal_k40_centroid + predicted_fwhm)
)

roi_half = (
    (centers > cal_k40_centroid - 0.5*predicted_fwhm) &
    (centers < cal_k40_centroid + 0.5*predicted_fwhm)
)

peak_area_1 = np.sum(calibrated_hist[roi])
peak_area_1_half = np.sum(calibrated_hist[roi_half])

efficiency_1 = peak_area_1 / len(raw)
efficiency_1_half = peak_area_1_half / len(raw)


# Method 2: calibrated + unsmeared
...
roi_unsmeared = (
    (unsmeared_centers > cal_k40_centroid - predicted_fwhm) &
    (unsmeared_centers < cal_k40_centroid + predicted_fwhm)
)

roi_half_unsmeared = (
    (unsmeared_centers > cal_k40_centroid - 0.5*predicted_fwhm) &
    (unsmeared_centers < cal_k40_centroid + 0.5*predicted_fwhm)
)

peak_area_2 = np.sum(unsmeared_hist[roi_unsmeared])
peak_area_2_half = np.sum(unsmeared_hist[roi_half_unsmeared])

efficiency_2 = peak_area_2 / len(raw)
efficiency_2_half = peak_area_2_half / len(raw)


# Method 3: raw events within +/- 5 keV
...
peak_area_3 = np.count_nonzero(raw_roi)
efficiency_3 = peak_area_3 / len(raw)


print("\nEFFICIENCY COMPARISON")
print("---------------------")

print("Method 1: calibrated + smeared")
print("  +/- 1 FWHM area:", peak_area_1)
print("  +/- 1 FWHM efficiency:", efficiency_1)
print("  +/- 0.5 FWHM area:", peak_area_1_half)
print("  +/- 0.5 FWHM efficiency:", efficiency_1_half)

print("\nMethod 2: calibrated + unsmeared")
print("  +/- 1 FWHM area:", peak_area_2)
print("  +/- 1 FWHM efficiency:", efficiency_2)
print("  +/- 0.5 FWHM area:", peak_area_2_half)
print("  +/- 0.5 FWHM efficiency:", efficiency_2_half)

print("\nMethod 3: raw +/- 5 keV")
print("  area:", peak_area_3)
print("  efficiency:", efficiency_3)
...
plt.hist(
    ...
    range=(0, 3000),
    histtype="step",
    label="Weighted + Smeared + Calibrated",
)

plt.axvspan(
    cal_k40_centroid - predicted_fwhm,
    cal_k40_centroid + predicted_fwhm,
    alpha=0.3,
    label="Peak ROI"
)
