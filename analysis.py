...
weight = np.exp(-(0.25*distance)/54.8)
...
#substitute coefficients depending on detector
a = -391.425599
b = 6.723136106
c = -0.000543789

bq_spec = bq.Spectrum.from_listmode(listmode_data=k40_smeared[mask], bins=edges, is_cal=True)
y = bq_spec.counts_vals
y_unc = np.sqrt(y)
dx = np.full_like(bq_spec.bin_centers_kev, bin_width, dtype=float)
bq_spec.plot(yscale="log", label="Weighted + Calibrated")
plt.xlabel("Energy (keV)")
plt.show()

#bq_test_spec.plot(yscale="log", xlim=(1000, 1800))
#plt.xlabel("Energy (keV)")
#plt.show()

model = bq.fitting.GaussModel(prefix="gauss0_") + bq.fitting.LineModel(prefix="linear_")

fitter = bq.Fitter(model, x=bq_spec.bin_centers_kev, y=y, y_unc=y_unc, dx=dx, roi=(1340, 1600))

fitter.fit(backend="lmfit")

#for some reason fitter.params contains different values from what you see in the plot and I don't understand why
params = fitter.params

print(params)

#this isn't working correctly right now
#print("Centroid:",fitter.params["gauss0_mu"].value, "keV")
#print("Sigma:", fitter.params["gauss0_sigma"].value, "keV")
#print("Peak Area:",fitter.params["gauss0_amp"].value, "counts")
#print("FWHM:", fitter.params["gauss0_fwhm"].value, "keV")
